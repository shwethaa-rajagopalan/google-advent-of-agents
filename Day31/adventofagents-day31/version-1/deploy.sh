...
if [ "$#" -lt 2 ]; then
    ...
    echo "MODEL_NAME can be 'gemini-3.1-flash-lite-preview' (default), 'gemini-2.5-pro', or 'gemini-2.5-flash'."
    exit 1
fi
...
# Validate model name
if [ "$MODEL_NAME" != "gemini-3.1-flash-lite-preview" ] && [ "$MODEL_NAME" != "gemini-2.5-pro" ] && [ "$MODEL_NAME" != "gemini-2.5-flash" ]; then
    echo "Invalid model name. Please use 'gemini-3.1-flash-lite-preview', 'gemini-2.5-pro', or 'gemini-2.5-flash'."
    exit 1
fi
